...
### 📋 **v2.4.0 업데이트 (2026-01-08)**

#### **모델별 검사대수/불량건수 차트 추가**
- **가압검사 및 제조품질 모두에 새로운 모델 분석 차트 추가**
- **월별 필터링 드롭다운**: 전체/1월~12월 선택 가능
- **TOP3 불량부품 호버 정보**: 각 모델별 주요 불량 부품 표시
- **불량률 자동 계산**: 검사대수 대비 불량건수 비율
...
#### **전체 합계 계산 방식 수정**
- **문제**: "전체" 선택 시 월별 합계와 다른 값 표시
- **원인**: 동일 S/N이 여러 달에 걸쳐 검사되는 경우 전체에서 한 번만 카운트
- **해결**: 월별로 각각 S/N 중복 제거 후 합산하여 월별 합계와 일치하도록 변경
- **검증**: GAIA-I DUAL 721대 확인 (기존 713대에서 수정)
...
- **v2.4 (2026년 1월)**: 모델별 검사대수/불량건수 차트 및 카운트 계산 수정

### 📈 **최근 변경사항**

#### **2026년 1월**
- ✅ **[1월 8일] 모델별 검사대수/불량건수 차트**: 가압검사 및 제조품질에 새로운 모델 분석 차트 추가
- ✅ **[1월 8일] 월별 필터링 드롭다운**: 전체/1월~12월 선택 가능
- ✅ **[1월 8일] TOP3 불량부품 호버 정보**: 각 모델별 주요 불량 부품 표시
- ✅ **[1월 8일] 전체 합계 계산 방식 수정**: 월별 S/N 중복 제거 후 합산하여 월별 합계와 일치하도록 변경
...
**최종 업데이트**: 2026년 1월 8일
**현재 버전**: v2.4.0 (모델별 검사대수/불량건수 차트 및 카운트 계산 수정)
