...
- **모델별 검사대수/불량건수 차트**: 가압검사 및 제조품질에 새로운 모델 분석 차트 추가
- **월별 필터링 드롭다운**: 전체/1월~12월 선택 가능한 드롭다운 메뉴
- **TOP3 불량부품 호버 정보**: 각 모델별 주요 불량 부품 표시
- **전체 합계 계산 방식 수정**: 월별 S/N 중복 제거 후 합산하여 월별 합계와 일치하도록 변경
- **검사대수 카운트 불일치 수정**: 동일 S/N이 여러 달에 걸쳐 검사되는 경우 처리 개선
...
- ✅ **ML 분석 데이터 분리**: 월간 ML은 2025년, 대시보드는 2026년 데이터 사용
