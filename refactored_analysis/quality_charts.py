...
class QualityCharts(BaseVisualizer):
    """제조품질 차트 생성 클래스"""

    def __init__(self):
        super().__init__()
        self.defect_data = None  # 호환성을 위한 속성 추가
        self.daily_inspection_data = None  # 날짜별 실적 데이터

    def load_quality_analysis_data(self) -> pd.DataFrame:
        """제조품질 불량분석 워크시트 데이터 로드"""
        if self.quality_analysis_data is None:
            self.quality_analysis_data = self._load_excel_data("제조품질 불량분석")
        return self.quality_analysis_data

    def load_quality_defect_data(self) -> pd.DataFrame:
        """제조품질 불량내역 워크시트 데이터 로드"""
        if self.quality_defect_data is None:
            self.quality_defect_data = self._load_excel_data("제조품질 불량내역")
        return self.quality_defect_data

    def load_defect_data(self) -> pd.DataFrame:
        """불량내역 워크시트 데이터 로드 (호환성을 위한 메서드)"""
        if self.defect_data is None:
            self.defect_data = self._load_excel_data("제조품질 불량내역")
        return self.defect_data

    def load_daily_inspection_data(self) -> pd.DataFrame:
        """날짜별 실적 워크시트 데이터 로드"""
        if self.daily_inspection_data is None:
            self.daily_inspection_data = self._load_excel_data("제조품질 날짜별 실적")
        return self.daily_inspection_data

    def extract_quality_kpi_data(self) -> dict:
        """제조품질 엑셀에서 KPI 데이터 추출 (특정 셀 직접 읽기)"""
        try:
            logger.info("📊 제조품질 KPI 데이터 추출 시작...")
            ...
            for sheet_name in workbook.sheetnames:
                if "제조품질" in sheet_name and "불량분석" in sheet_name:
            ...
            logger.info(
                f"✅ 제조품질 KPI 데이터 추출 완료: CH수={kpi_data['total_ch']}, 불량건수={kpi_data['total_defects']}, 불량률={kpi_data['avg_rate']}%"
            )
            return kpi_data

        except Exception as e:
            logger.error(f"❌ 제조품질 KPI 데이터 추출 실패: {e}")
            # 실패 시 월별 데이터로 대체
            try:
                ...
                logger.info(f"📊 제조품질 대체 데이터 사용: {fallback_data}")
                return fallback_data
            except:
                logger.error("❌ 제조품질 대체 데이터도 실패")
    ...
    def extract_quality_monthly_data(self) -> Dict:
        ...
        try:
            ...
            if self.quality_analysis_data is None:
                self.load_quality_analysis_data()
    ...
    def create_quality_action_integrated_chart(self) -> go.Figure:
        """제조품질 조치 유형별 통합 차트 (드롭다운 메뉴 포함)"""
        try:
            logger.info("📊 제조품질 조치유형별 통합 차트 생성 (불량내역 기반)")

            # 불량내역 데이터 로드
            if self.quality_defect_data is None:
                self.load_quality_defect_data()
            ...
            if (
                df.empty
                or "발생일" not in df.columns
                or "상세조치내용" not in df.columns
            ):
                logger.warning("⚠️ 제조품질 불량내역 데이터가 없거나 필수 컬럼이 누락됨")
                # 기본 차트 반환
                fig = go.Figure()
                fig.add_trace(
                    go.Bar(
                ...
                if action in quarterly_data.columns:
                    ...
                    for quarter_period in quarterly_data.index:
                        ...
                        top_parts = (
                            ...
                            .head(5)
                            .index.tolist()
                            if "부품명" in quarter_data_filtered.columns
                            else []
                        )
                        hover_text = (
            ...
            for i, action in enumerate(top_actions):
                if action in monthly_data.columns:
                    # 각 월+조치유형 조합의 주요 부품명 추출 (hover용)
                    monthly_hover_texts = []
                    for month_period in monthly_data.index:
                        month_data_filtered = df_top3[
                            (df_top3["발생월"] == month_period)
                            & (df_top3["상세조치내용"] == action)
                        ]
                        top_parts = (
                            month_data_filtered["부품명"]
                            .value_counts()
                            .head(5)
                            .index.tolist()
                            if "부품명" in month_data_filtered.columns
                            else []
                        )
                        hover_text = (
                            f"주요부품: {', '.join(top_parts[:3])}"
                        ...
                        monthly_hover_texts.append(hover_text)
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            + "건수: %{y}<br>"
                            + "%{customdata}<extra></extra>",
                            customdata=monthly_hover_texts,
                            visible=False,  # 기본 숨김
                            showlegend=True,
                        )
                    )
            ...
            # 전체분포 버튼
            total_visibility = [True] + [False] * (len(top_actions) * 2)
            dropdown_buttons.append(
                dict(
                    label="전체 분포",
                    method="update",
                    args=[
                        {"visible": total_visibility},
                        {
            ...
            # 분기별 비교 버튼
            quarterly_visibility = (
                [False] + [True] * len(top_actions) + [False] * len(top_actions)
            )
            dropdown_buttons.append(
                dict(
                    label="분기별 비교 (TOP3)",
            ...
            logger.info("✅ 제조품질 조치유형별 통합 차트 생성 완료 (드롭다운 포함)")
            return fig

        except Exception as e:
            logger.error(f"❌ 제조품질 조치유형별 통합 차트 생성 실패: {e}")
            flush_log(logger)
            raise

    def create_quality_supplier_integrated_chart(self) -> go.Figure:
        """제조품질 외주사별 통합 차트 (간소화 버전)"""
        try:
            # 기본 데이터
            suppliers = ["TMS(기구)", "C&A", "P&S"]
            supplier_counts = [28, 25, 17]
            ...
            fig = go.Figure(
                data=[
                    go.Bar(
                        x=suppliers,
                        y=supplier_counts,
                        text=[f"{count}건" for count in supplier_counts],
                        textposition="auto",
                        textfont=dict(size=14, color="white"),
                        marker_color=self.generate_colors(len(suppliers)),
            ...
            logger.error(f"❌ 제조품질 외주사별 차트 생성 실패: {e}")
            flush_log(logger)
            raise

    def create_quality_part_monthly_chart(self) -> go.Figure:
        """제조품질 부품별 월별 차트 (간소화 버전)"""
        try:
            if self.quality_defect_data is None:
                self.load_quality_defect_data()
            ...
            fig.update_layout(
                title=dict(
                    text="제조품질 주요 부품별 월별 불량 추이", x=0.5, xanchor="center"
                ),
                xaxis_title="월",
                yaxis_title="건수",
                height=450,
            ...
            logger.error(f"❌ 제조품질 부품별 월별 차트 생성 실패: {e}")
            flush_log(logger)
            raise

    def extract_supplier_data(self) -> Dict:
        """제조품질 외주사별 불량 데이터 추출 (동적)"""
        try:
            if self.quality_analysis_data is None:
                self.load_quality_analysis_data()
            ...
            for idx, row in self.quality_analysis_data.iterrows():
                if pd.notna(row.iloc[1]) and (
                    "외주사별" in str(row.iloc[1]) or "협력사별" in str(row.iloc[1])
                ):
                            ...
                            if rate == 0:
                                ...
                                for col_idx in range(
                                    ...
                                    if pd.notna(cell_value) and isinstance(
                                        ...
                                        rate_values.append(float(cell_value) * 100)
                                if rate_values:
            ...
            # 데이터가 없으면 기본값 사용
            if not suppliers:
                logger.warning("⚠️ 동적 제조품질 외주사 데이터 추출 실패, 기본값 사용")
                suppliers = ["동양전관", "금성정밀", "태성전기"]
                supplier_counts = [25, 18, 12]
                supplier_rates = [45.5, 32.7, 21.8]

            logger.info(
                f"📊 제조품질 동적 외주사 데이터 추출 완료: {len(suppliers)}개 업체"
            )
            ...
            logger.error(f"❌ 제조품질 외주사 데이터 추출 실패: {e}")
            flush_log(logger)
            raise
    ...
    def create_supplier_integrated_chart(self) -> go.Figure:
        ...
        try:
            ...
            if self.defect_data is not None:
                logger.info(
                    f"📊 제조품질 불량내역 데이터 컬럼: {list(self.defect_data.columns)}"
                )
                logger.info(
                    f"📊 제조품질 불량내역 데이터 크기: {len(self.defect_data)}건"
                )
            ...
            for i, (supplier, count, rate) in enumerate(
                ...
                if self.defect_data is not None:
                        ...
                        if col in self.defect_data.columns:
                            ...
                            if (
                                ...
                                if col in ["작업자", "조치자(외주)"]:
                                    # TMS(전장)이면 전장작업불량만, TMS(기구)면 기구작업불량만
                                    if "TMS(전장)" in supplier:
                                        col_df = col_df[
                                            col_df["대분류"].str.contains(
                                                "전장작업불량", case=False, na=False
                                            )
                                        ]
                                    elif "TMS(기구)" in supplier:
                                        col_df = col_df[
                                            col_df["대분류"].str.contains(
                                    ...
                                    # 외주사, 협력사 컬럼의 경우 TMS는 제외 (작업자/조치자 기준만 사용)
                                    col_df = pd.DataFrame()
                    ...
                    logger.info(
                        f"📊 제조품질 {supplier} 외주사 데이터: 총 {len(supplier_df)}건, 사용 컬럼: {', '.join(used_columns)}"
                    )
            ...
            for i, (supplier, rates) in enumerate(
                ...
                for j, (quarter, rate) in enumerate(
                    ...
                    if self.defect_data is not None:
                        ...
                        logger.info(
                            f"📊 제조품질 {supplier} {quarter} 데이터: {len(supplier_quarter_df)}건"
                        )
    ...
    def create_quality_part_integrated_chart(self) -> go.Figure:
        ...
        try:
            logger.info("📊 제조품질 부품별 통합 차트 생성 (드롭다운 형태)...")

            # 제조품질 불량내역 데이터 로드
            defect_data = self.load_quality_defect_data()
            ...
            # 유효한 데이터만 필터링
            df_filtered = df.dropna(subset=["발생분기", "부품명"])
            ...
            for q_idx, quarter in enumerate(quarters):
                ...
                for p_idx, (part, count) in enumerate(quarter_top5.items()):
                    ...
                    action_details = (
                        ...
                        if "상세조치내용" in quarter_part_df.columns
                        else []
                    )
                    location_details = (
                        quarter_part_df["불량위치"].dropna().unique()[:5]
                        if "불량위치" in quarter_part_df.columns
                        else []
                    )
                ...
                dropdown_buttons.append(
                    dict(
                        ...
                        args=[
                            {"visible": visibility},
                            {
                                "title": f"제조품질 {quarter_name} TOP5 부품 불량 현황",
            ...
            dropdown_buttons.append(
                dict(
                    ...
                    args=[
                        ...
                        {
                            "title": "제조품질 전체 기간 TOP3 부품 월별 추이",
                            "xaxis": {"title": "월", "visible": True},
                            "yaxis": {"title": "불량 건수", "visible": True},
                            "showlegend": True,
                        },
            ...
            fig.update_layout(
                title=dict(
                    text="제조품질 부품별 전체 분포 (TOP10)", x=0.5, xanchor="center"
                ),
                xaxis=dict(
            ...
            logger.info("✅ 제조품질 부품별 통합 차트 생성 완료 (드롭다운 형태)")
            return fig

        except Exception as e:
            logger.error(f"❌ 제조품질 부품별 통합 차트 생성 실패: {e}")
            flush_log(logger)
            raise
    ...
    def extract_model_inspection_defect_data(self, month: int = None) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"📊 제조품질 모델별 검사대수/불량건수 데이터 추출 시작... ({month_str})"
            )

            # 1. 검사대수 데이터 로드 (제조품질 날짜별 실적)
            self.load_daily_inspection_data()
            df_inspection = self.daily_inspection_data.copy()

            # 2. 불량건수 데이터 로드 (제조품질 불량내역)
            self.load_quality_defect_data()
            df_defect = self.quality_defect_data.copy()
            ...
            logger.info(
                f"✅ 제조품질 모델별 데이터 추출 완료 ({month_str}): TOP {len(top_models)}개 모델"
            )

            return result

        except Exception as e:
            logger.error(f"❌ 제조품질 모델별 데이터 추출 실패: {e}")
            flush_log(logger)
            raise

    def create_model_inspection_defect_chart(self) -> go.Figure:
        """제조품질 모델별 검사대수 및 불량건수 비교 차트 생성 (드롭다운 메뉴 포함)"""
        try:
            logger.info(
                "📊 제조품질 모델별 검사대수/불량건수 차트 생성 시작 (드롭다운 포함)..."
            )
                ...
                dropdown_buttons.append(
                    dict(
                        ...
                        args=[
                            {"visible": visibility},
                            {
                                "title": f"제조품질 모델별 검사대수 및 불량건수 비교 - {period} (TOP10)"
                            },
                        ],
                    )
                )
            ...
            fig.update_layout(
                title={
                    "text": "제조품질 모델별 검사대수 및 불량건수 비교 - 전체 (TOP10)",
                    "x": 0.5,
                    "xanchor": "center",
                    "font": {"size": 20, "family": "Arial, sans-serif"},
                },
            ...
            logger.info(
                f"✅ 제조품질 모델별 검사대수/불량건수 차트 생성 완료 (드롭다운: {len(periods)}개 옵션)"
            )
            return fig

        except Exception as e:
            logger.error(f"❌ 제조품질 모델별 차트 생성 실패: {e}")
            flush_log(logger)
            raise
